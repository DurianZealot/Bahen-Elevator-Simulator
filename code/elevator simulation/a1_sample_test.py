...
def test_file_arrival_generator_2() -> None:
    max_floor = 6
    file_generator = FileArrivals(max_floor, 'sample_arrival_floor1_round0.csv')

    # First try round 1. Note that round numbering starts at 0,
    # but there are no arrivals at round 0 according to the sample file.
    round_one = file_generator.generate(1)
    for _, people in round_one.items():
        assert people == []

    # Second try round 0. There is arrivals for round 1
    round_zero = file_generator.generate(0)
    for floor, people in round_zero.items():
        if floor == 1:
            assert len(people) == 5
            assert people[0].target == 4
            assert people[1].target == 2
            assert people[2].target == 3
            assert people[3].target == 5
            assert people[4].target == 6
